...
init_per_suite(Config) ->
    ...
    case rabbit_ct_helpers:run_setup_steps(?MODULE, Config1) of
        {skip, _} = Error ->
            Error;
        Config2 ->
...
setup_file_handle_cache(Config) ->
    ok = rabbit_ct_broker_helpers:run_on_broker(
      ?config(rmq_nodename, rabbit_ct_helpers:get_node_config(Config, 0)),
...
init_per_group(backing_queue_tests, Config) ->
    Node = ?config(rmq_nodename, rabbit_ct_helpers:get_node_config(Config, 0)),
...
app_management1(_Config) ->
    rabbit_ct_helpers:control_action(wait, [os:getenv("RABBITMQ_PID_FILE")]),
    ...
    ok = rabbit_ct_helpers:control_action(trace_on, []),
    ok = rabbit_ct_helpers:control_action(stop_app, []),
    ok = rabbit_ct_helpers:control_action(stop_app, []),
    ok = rabbit_ct_helpers:control_action(status, []),
    ok = rabbit_ct_helpers:control_action(cluster_status, []),
    ok = rabbit_ct_helpers:control_action(environment, []),
    ok = rabbit_ct_helpers:control_action(start_app, []),
    ok = rabbit_ct_helpers:control_action(start_app, []),
    ok = rabbit_ct_helpers:control_action(status, []),
    ok = rabbit_ct_helpers:control_action(report, []),
    ok = rabbit_ct_helpers:control_action(cluster_status, []),
    ok = rabbit_ct_helpers:control_action(environment, []),
    ok = rabbit_ct_helpers:control_action(trace_off, []),
    passed.
...
list_operations_timeout_pass1(Config) ->
    ...
    %% list users
    ok = rabbit_ct_helpers:control_action(add_user,
      ["list_operations_timeout_pass-user",
       "list_operations_timeout_pass-password"]),
    {error, {user_already_exists, _}} =
        rabbit_ct_helpers:control_action(add_user,
          ["list_operations_timeout_pass-user",
           "list_operations_timeout_pass-password"]),
    ...
    %% list vhosts
    ok = rabbit_ct_helpers:control_action(add_vhost,
      ["/list_operations_timeout_pass-vhost"]),
    {error, {vhost_already_exists, _}} =
        rabbit_ct_helpers:control_action(add_vhost,
          ["/list_operations_timeout_pass-vhost"]),
    ...
    ok = rabbit_ct_helpers:control_action_t(list_user_permissions,
      ["list_operations_timeout_pass-user"],
      ?TIMEOUT_LIST_OPS_PASS),

    %% list policies
    ok = rabbit_ct_helpers:control_action_opts(
      ["set_policy", "list_operations_timeout_pass-policy", ".*",
       "{\"ha-mode\":\"all\"}"]),
    ok = rabbit_ct_helpers:control_action_t(list_policies, [],
                                            ?TIMEOUT_LIST_OPS_PASS),
    ok = rabbit_ct_helpers:control_action(clear_policy,
    ...
    P = ?config(tcp_port_amqp, rabbit_ct_helpers:get_node_config(Config, 0)),
    ...
    %% do some cleaning up
    ok = rabbit_ct_helpers:control_action(delete_user,
      ["list_operations_timeout_pass-user"]),
    {error, {no_such_user, _}} =
        rabbit_ct_helpers:control_action(delete_user,
          ["list_operations_timeout_pass-user"]),

    ok = rabbit_ct_helpers:control_action(delete_vhost,
      ["/list_operations_timeout_pass-vhost"]),
    {error, {no_such_vhost, _}} =
        rabbit_ct_helpers:control_action(delete_vhost,
          ["/list_operations_timeout_pass-vhost"]),

    %% close_connection
    Conns = rabbit_ct_broker_helpers:get_connection_pids([C1, C2]),
    [ok, ok] = [ok = rabbit_ct_helpers:control_action(
...
user_management1(_Config) ->
    ...
    {error, {no_such_user, _}} =
        rabbit_ct_helpers:control_action(change_password,
          ["user_management-user", "user_management-password"]),
    {error, {no_such_vhost, _}} =
        rabbit_ct_helpers:control_action(delete_vhost,
          ["/user_management-vhost"]),
    {error, {no_such_user, _}} =
        rabbit_ct_helpers:control_action(set_permissions,
          ["user_management-user", ".*", ".*", ".*"]),
    {error, {no_such_user, _}} =
        rabbit_ct_helpers:control_action(clear_permissions,
          ["user_management-user"]),
    {error, {no_such_user, _}} =
        rabbit_ct_helpers:control_action(list_user_permissions,
          ["user_management-user"]),
    {error, {no_such_vhost, _}} =
        rabbit_ct_helpers:control_action(list_permissions, [],
          [{"-p", "/user_management-vhost"}]),
    {error, {invalid_regexp, _, _}} =
        rabbit_ct_helpers:control_action(set_permissions,
          ["guest", "+foo", ".*", ".*"]),
    {error, {no_such_user, _}} =
        rabbit_ct_helpers:control_action(set_user_tags,
    ...
    {error, {user_already_exists, _}} =
        rabbit_ct_helpers:control_action(add_user,
          ["user_management-user", "user_management-password"]),
    ok = rabbit_ct_helpers:control_action(clear_password,
      ["user_management-user"]),
    ok = rabbit_ct_helpers:control_action(change_password,
      ["user_management-user", "user_management-newpassword"]),

    TestTags = fun (Tags) ->
                       Args = ["user_management-user" | [atom_to_list(T) || T <- Tags]],
                       ok = rabbit_ct_helpers:control_action(set_user_tags, Args),
                       {ok, #internal_user{tags = Tags}} =
                           rabbit_auth_backend_internal:lookup_user(
                             <<"user_management-user">>),
                       ok = rabbit_ct_helpers:control_action(list_users, [])
               end,
    ...
    %% user authentication
    ok = rabbit_ct_helpers:control_action(authenticate_user,
      ["user_management-user", "user_management-newpassword"]),
    {refused, _User, _Format, _Params} =
        rabbit_ct_helpers:control_action(authenticate_user,
          ["user_management-user", "user_management-password"]),
    ...
    {error, {vhost_already_exists, _}} =
        rabbit_ct_helpers:control_action(add_vhost,
          ["/user_management-vhost"]),
    ok = rabbit_ct_helpers:control_action(list_vhosts, []),
    ...
    ok = rabbit_ct_helpers:control_action(set_permissions,
      ["user_management-user", ".*", ".*", ".*"],
      [{"-p", "/user_management-vhost"}]),
    ok = rabbit_ct_helpers:control_action(list_permissions, [],
      [{"-p", "/user_management-vhost"}]),
    ok = rabbit_ct_helpers:control_action(list_permissions, [],
      [{"-p", "/user_management-vhost"}]),
    ok = rabbit_ct_helpers:control_action(list_user_permissions,
      ["user_management-user"]),

    %% user/vhost unmapping
    ok = rabbit_ct_helpers:control_action(clear_permissions,
...
runtime_parameters1(_Config) ->
    ...
    Good = fun(L) -> ok                = rabbit_ct_helpers:control_action(set_parameter, L) end,
    Bad  = fun(L) -> {error_string, _} = rabbit_ct_helpers:control_action(set_parameter, L) end,
    ...
    ok = rabbit_ct_helpers:control_action(list_parameters, []),

    ok = rabbit_ct_helpers:control_action(clear_parameter, ["test", "good"]),
    ok = rabbit_ct_helpers:control_action(clear_parameter, ["test", "maybe"]),
    ok = rabbit_ct_helpers:control_action(clear_parameter, ["test", "admin"]),
    {error_string, _} =
        rabbit_ct_helpers:control_action(clear_parameter, ["test", "neverexisted"]),
...
policy_validation1(_Config) ->
    ...
    SetPol = fun (Key, Val) ->
                     rabbit_ct_helpers:control_action_opts(
    ...
    ok = rabbit_ct_helpers:control_action(clear_policy, [PolicyName]),
...
policy_opts_validation1(_Config) ->
    ...
    Set  = fun (Extra) -> rabbit_ct_helpers:control_action_opts(
                            ["set_policy", PolicyName,
                             ".*", "{\"ha-mode\":\"all\"}"
                             | Extra]) end,
    OK   = fun (Extra, Props) ->
...
ha_policy_validation1(_Config) ->
    ...
    Set  = fun (JSON) -> rabbit_ct_helpers:control_action_opts(
                           ["set_policy", PolicyName,
                            ".*", JSON]) end,
    OK   = fun (JSON, Def) ->
               ok = Set(JSON),
...
server_status1(Config) ->
    ...
    %% list queues
    ok = rabbit_ct_helpers:info_action(list_queues, rabbit_amqqueue:info_keys(), true),

    %% as we have no way to collect output of
    %% rabbit_ct_helpers:info_action/3 call, the only way we can
    %% test individual queueinfoitems is by directly calling
    %% rabbit_amqqueue:info/2
    [{exclusive, false}] = rabbit_amqqueue:info(Q, [exclusive]),
    [{exclusive, true}] = rabbit_amqqueue:info(Q2, [exclusive]),

    %% list exchanges
    ok = rabbit_ct_helpers:info_action(list_exchanges, rabbit_exchange:info_keys(), true),

    %% list bindings
    ok = rabbit_ct_helpers:info_action(list_bindings, rabbit_binding:info_keys(), true),
    ...
    timer:sleep(100),
    ok = rabbit_ct_helpers:info_action(list_connections,
                     rabbit_networking:connection_info_keys(), false),
    %% close_connection
    [ConnPid] = rabbit_ct_broker_helpers:get_connection_pids([C]),
    ok = rabbit_ct_helpers:control_action(close_connection,
      [rabbit_misc:pid_to_string(ConnPid), "go away"]),

    %% list channels
    ok = rabbit_ct_helpers:info_action(list_channels, rabbit_channel:info_keys(), false),

    %% list consumers
    ok = rabbit_ct_helpers:control_action(list_consumers, []),

    %% set vm memory high watermark
    HWM = vm_memory_monitor:get_vm_memory_high_watermark(),
    ok = rabbit_ct_helpers:control_action(set_vm_memory_high_watermark, ["1"]),
    ok = rabbit_ct_helpers:control_action(set_vm_memory_high_watermark, ["1.0"]),
    %% this will trigger an alarm
    ok = rabbit_ct_helpers:control_action(set_vm_memory_high_watermark, ["0.0"]),
    %% reset
    ok = rabbit_ct_helpers:control_action(set_vm_memory_high_watermark, [float_to_list(HWM)]),

    %% eval
    {error_string, _} = rabbit_ct_helpers:control_action(eval, ["\""]),
    {error_string, _} = rabbit_ct_helpers:control_action(eval, ["a("]),
    ok = rabbit_ct_helpers:control_action(eval, ["a."]),
...
memory_high_watermark1(_Config) ->
    ...
    HWM = vm_memory_monitor:get_vm_memory_high_watermark(),
    %% this will trigger an alarm
    ok = rabbit_ct_helpers:control_action(set_vm_memory_high_watermark,
      ["absolute", "2000"]),
...
set_disk_free_limit_command1(_Config) ->
    ok = rabbit_ct_helpers:control_action(set_disk_free_limit, ["2000kiB"]),
    2048000 = rabbit_disk_monitor:get_disk_free_limit(),
    ok = rabbit_ct_helpers:control_action(set_disk_free_limit, ["mem_relative", "1.1"]),
    ...
    ok = rabbit_ct_helpers:control_action(set_disk_free_limit, ["50MB"]),
    passed.
...
delegates_async(Config) ->
    ...
    From = ?config(rmq_nodename, rabbit_ct_helpers:get_node_config(Config, I)),
    To = ?config(rmq_nodename, rabbit_ct_helpers:get_node_config(Config, J)),
